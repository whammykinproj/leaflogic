...
function LoginForm() {
  ...
  const handleGoogleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/scout/api/auth/callback`,
      },
    });
    if (error) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-zinc-100 text-zinc-900 font-medium py-3 px-4 rounded-lg transition"
          >
        ...
        <p className="text-xs text-zinc-500 text-center mt-6">
          ...
          <Link href="/scout/terms" className="underline hover:text-zinc-300">
            Terms
          </Link>{" "}
          and{" "}
          <Link href="/scout/privacy" className="underline hover:text-zinc-300">
            Privacy Policy
          </Link>
          .
        </p>
